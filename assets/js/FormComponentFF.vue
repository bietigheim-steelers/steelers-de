<template>
  <GroupElement
    name="ff"
    v-if="Date.now() < Date.parse('22 May 2025 23:59:59 GMT')"
    :conditions="[
      ['ticket_category', '!=', null],
      ['ticket_category', '!=', 'jugendlich'],
      ['ticket_category', '!=', 'kind'],
      ['ticket_category', '!=', 'familie1'],
      ['ticket_category', '!=', 'familie2'],
      ['ticket_category', '!=', 'familie3'],
    ]"
  >
    <RadiogroupElement
      @change="onFFChange"
      ...
      :items="[
        {
          value: 'werben',
          label: 'Ich will jemanden werben',
          description: '(zur Ablage im Smartphone)',
        },
    ...
    <TextElement
      name="ff_new_dk"
      rules="required"
      placeholder=" Name des neuen DK Inhabers"
      :conditions="[['ff.family_and_friends', '==', 'werben']]"
    />
    <TextElement
      name="ff_old_dk"
      rules="required"
      placeholder=" Name des werbenden DK Inhabers"
...
export default {
  setup(props, context) {
    ...
    const ticketFormItems = computed(() => {
      if (
        form$.value.data.family_and_friends == "werben" ||
        form$.value.data.family_and_friends == "geworben"
      ) {
        return [
          { value: "plastik", label: "Klassische Plastikkarte" },
          {
            value: "mobile_plastik",
            label: "Klassische Plastikkarte + mobile Dauerkarte",
          },
        ];
      }
      return [

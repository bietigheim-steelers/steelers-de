    ...
    const ticket_items = computed(() => {
      let items = [
        ...
        },
        {
          value: "mitglied",
          label:
            "SC Mitglied / PURE Mitglied <small>" +
            getPriceText(
              form$.value.data.ticket_type,
              "ermaessigt",
              form$.value.data.ticket_area
        ...
        },
        {
          value: "jugendlich",
          label:
            "Jugendlicher (13-17 Jahre) <small>" +
            getPriceText(
              form$.value.data.ticket_type,
              "jugendlich",
              form$.value.data.ticket_area
        ...
        },
        {
          value: "behinderung",
          label:
            "Fan mit Behinderung ab 50% <small>" +
            getPriceText(
              form$.value.data.ticket_type,
              "behinderung",
              form$.value.data.ticket_area
        ...
        items.push(
          {
            value: "familie1",
            label:
              "Familienkarte 1 (1 x Vollzahler + 2 Kinder/Jugendliche) <small>" +
              getPriceText(form$.value.data.ticket_type, "familie1", "C") +
              "</small>",
          },
          {
            value: "familie2",
            label:
              "Familienkarte 2 (2 x Vollzahler + 1 Kind/Jugendlicher) <small>" +
              getPriceText(form$.value.data.ticket_type, "familie2", "C") +
              "</small>",
          },
          {
            value: "familie3",
            label:
              "Familienkarte 3 (2 x Vollzahler + 2 Kinder/Jugendliche) <small>" +
              getPriceText(form$.value.data.ticket_type, "familie3", "C") +
              "</small>",
          }
        );
      }

<template>
  <v-group v-if="!row.skip || (row.skip && !row.skip.includes(seat))">
    <v-circle
      :radius="seat_size.w / 2"
      :fill="colors.circle"
      :strokeWidth="0.5"
      :stroke="colors.circle_stroke"
      :x="x"
      :y="y"
    ></v-circle>
    <v-text
...
export default {
  ...
  setup(props) {
    ...
    const selected = computed(() => {
      ...
      if (
        ...
        if (
          ...
          return (
            form$.value.data.seat_block === section.id &&
            form$.value.data.seat_row === seatRow &&
            (form$.value.data.seat_seat == seatLabel ||
              form$.value.data.seat_seat == parseInt(seatLabel) - 1 ||
              form$.value.data.seat_seat == parseInt(seatLabel) - 2)
          );
        }
        if (form$.value.data.ticket_category == "familie3") {
          return (
            ...
            (form$.value.data.seat_seat == seatLabel ||
              form$.value.data.seat_seat == parseInt(seatLabel) - 1 ||
              form$.value.data.seat_seat == parseInt(seatLabel) - 2 ||
              form$.value.data.seat_seat == parseInt(seatLabel) - 3)
          );
        }
      }
      return (
        seatId ===
        `${form$.value.data.seat_block}_${form$.value.data.seat_row}_${form$.value.data.seat_seat}`
      );
    });
    ...
    onMounted(async () => {
      ...
      booked.value = seatsData.booked.includes(seatId);
      reserved.value = seatsData.reserved.includes(seatId);
    ...
    const clickHandler = () => {
      ...
      if (
        form$.value.data.ticket_category &&
        form$.value.data.ticket_category.includes("familie") &&
        (seatLabel == row.seats - row.skip.length ||
          seatLabel == row.seats - 1 - row.skip.length)
      ) {
        return;
      }
      if (
        ...
        (seatLabel == row.seats ||
          seatLabel == row.seats - 1 ||
          seatLabel == row.seats - 2)
      ) {
        return;
      }
      ...
      if (
        form$.value.data.ticket_category &&
        form$.value.data.ticket_category.includes("familie3") &&
        [seatLabel, seatLabel + 1, seatLabel + 2, seatLabel + 3].some((label) =>
          bookedSeats.value.includes(
            `${section.id}_${row.rowLabel || rowNumber}_${label}`
          )
        )
      ) {
        return;
      ...
      if (
        ...
        (form$.value.data.ticket_category == "familie1" ||
          form$.value.data.ticket_category == "familie2") &&
        [seatLabel, seatLabel + 1, seatLabel + 2].some((label) =>
          bookedSeats.value.includes(
            `${section.id}_${row.rowLabel || rowNumber}_${label}`
          )
        )
